%% DESCRIPTION
% Like wkspc_construction but for rescan data
%
%% HISTORY
% - v1 created by Dan Palken on 3/24/20. Based directly on wkspc_construction_v15
% - v1 created by Dan Palken on 3/24/20. v2 is to v1 as wkspc_construction_v16 is to 
% wkspc_construction_v15: that is, it is written for meta-processing
% 
%% REFERENCES
...
%% ============================================================================================== %%
%% USER INPUT
ver = 'v2'; % should match script name. Not for the data to be loaded
% data_run = '20200319';
% dv_main = '0'; % data version
...
f_lo_spec_Hz = 45E3;
f_hi_spec_Hz = 605E3;% 60 kHz more than needed
...
VF.crit.Gs_min_1q_pow = 0.99; % essentially, set to 1 for squeezer-off rescans. Will enforce 1 later
VF.crit.Gs_max_1q_pow = 1.01;

% range made 50% wider to accomodate having only 1 shot to get a good cal for rescans
VF.hot_mult_min = 0.85;
VF.hot_mult_max = 1.15; % nice to construct the max and min roughly symmetrically around 1

VF.GA_addnoise_scale_min_1q_dB = 20; 
VF.GA_addnoise_scale_max_1q_dB = 30.5; 
VF.GA_spec_fixed_1q_dB = pow2db(650); 

VF.n_hot_mults = 149; % this must be odd
...
%% ============================================================================================== %%
%% OUTPUT DIRECTORY
output_folder = ['HAYSTACp2_rescan_raw_saved_', data_run, '_', ver];
% create directory if necessary:
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end

fig_folder = ['HAYSTACp2_rescan_wkspc_construction_figs_', data_run, '_', ver];
% create directory if necessary:
...
par = load(['Rescans', f, data_run, f, par_wkspc]).par; % this struct contains all the meta-data
...
temp_tx1 = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_1_', tx1_suf, '.mat']); 
temp_spec = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_1_', spec_suf, '.mat']); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% refit couplings:
rc_rescan = load([rc_dir, f, 'datebetaQinfo_rescan', '.mat']).datebetaQinfo;
rc.beta = rc_rescan.beta;
rc.spec_date = rc_rescan.spectrum_date;
rc.spec_idx = rc_rescan.spectrum_it;
rc.Q = rc_rescan.Q;
...
for i = 1:n_OPs+1
    if exist(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', abg_suf, '.mat'], 'file')
        abg_exists(i) = 1; 
    end 
    if exist(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', add_noise_suf, '.mat'], ...
            'file')
...
while counting_i <= length(order)
    ...
    tx1 = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', tx1_suf, '.mat']); 
    tx2 = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', tx2_suf, '.mat']); 
    rf = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', rf_suf, '.mat']); 
    spec = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', spec_suf, '.mat']); 
    G_AMP = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i), '_', G_AMP_suf, '.mat']); 
    ...
    OP_data.direct_squeezing = par(i).squeezing; % set to 100 for rscans
    ...
    n_groups = length(spec.meanavgps.pt_power_est_list);
    n_subs_per_group = length(spec.meanavgps.pt_power_est_list{1});
    tone_heights = NaN(n_groups, n_subs_per_group);
    for y = 1:n_groups
        tone_heights(y,:) = spec.meanavgps.pt_power_est_list{y};
    end
    OP_data.tone_height = mean(tone_heights, 'all');
    OP_data.tone_height_std = (std(tone_heights, [], 'all'));
    ...
    if isempty(betaQ_ind) % FLAG - this is just to tide the code over until all rescans are included in the refit couplings
        beta = (par(i).coupling_factor + par(i).coupling_factor2) / 2;
        Q_load = (par(i).Cavity_Q + par(i).Cavity_Q2)/2;
        warning('refit couplings missing - loading par file couplings')
    else
    ...
    if is_full_cal_op % is an operating point where we perfom a full cal. These come first 
        ...
        add_noise = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i+1), '_', ...
            add_noise_suf, '.mat']);
        % order as [cold, hot]
        ...
        % code Danielle wrote for these multiplies back in the mean of all the gain tones. I will
        % take these gains out - not by taking the measured gains themselves out, but by taking out
        % the probe tone itself. This applies WHETHER OR NOT we are using the pt_monitor values, in
        % fact, since the non-pt_monitored ones will also need to be g ain-normalized for the
        ...
        pt_addnoise_pow = pt_addnoise_pow / pt_addnoise_pow(1); % do this by dividng out 1st element
...
%         std_pt_monhot = std(pts_monhot, [], 'all');
        ...
        [OP_data.f4NH_IF_Hz, VF.N_Hp_test_1q_qta, ~] = FUNC_analyze_add_noise_v13(OP_data, i, ...
            ...
            add_noise.f_GHzcold, add_noise.I_cold, add_noise.Q_cold, add_noise.f_GHzhot, ...
            add_noise.I_hot, add_noise.Q_hot, save_figs, fig_folder);
        
        % abg calibrations for hot road and squeezing:
        % + 1 in file name because cals actually belong to the previous OP
        abg = load(['Rescans', f, data_run, f, data_run '_', dv_main, '_', num2str(i+1), '_', abg_suf, '.mat']); 
        ...
        abg_test_out = FUNC_proc_abg_rescan_v1(UNIV, OP_data, abg, i, abg_gain_tone_det_MHz, VF, ...
        ...
        select_params = FUNC_valfct_rescan_v1(UNIV, OP_data, i, abg_test_out, VF, gain_out, save_figs, ...
            fig_folder);
