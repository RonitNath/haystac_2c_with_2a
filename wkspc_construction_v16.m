...
% - v16 created by DP on 1/25/20. Version that gets called by a meta-processing function. Has some
% variables promoted to globals
%
%% REFERENCES
...
%% ============================================================================================== %%
%% GLOBAL
global chi_mm2_fcn data_run dv_main
chi_mm2_fcn = @(kap_dif, kap_sum, det) (kap_dif^2 + 4*det.^2)./(kap_sum^2 + 4*det.^2);

%% ============================================================================================== %%
%% USER INPUT
ver = 'v16'; % should match script name. Not for the data to be loaded
% data_run = '20191111'; - commented out and set globally now 
% dv_main = '0'; % data version - commented out and set globally now 
...
VF.crit.pmin = 0.1; % min allowed participation ratio
VF.crit.pmax = 0.8; % max allowed participation ratio
...
rc_1 = load([rc_dir, f, 'datebetaQinfo', '.mat']).datebetaQinfo; 
rc.beta = [rc_1.beta];
rc.spec_date = [rc_1.spectrum_date];
rc.spec_idx = [rc_1.spectrum_it];
rc.Q = [rc_1.Q];
...
%         std_pt_monhot = std(pts_monhot, [], 'all');
        ...
        abg_test_out = FUNC_proc_abg_v5(UNIV, OP_data, abg, i, abg_gain_tone_det_MHz, VF, ...
        ...
        % manually setting the lower limit on the S_c minimum to be 0.38
        % since we know that the hot rod temperature ends up being around
        % 225 mK - Alex L Sept 15, 2021
        %Mod Flag !@@@@!
        % VF.crit.Scmin_1q_qta = 0.38;
