function [output_spec, output_spec_nanless] = FUNC_sgolayfilt_omitnan_v7(input_spec, ...
order, frame_len, freq_domain_fit)
%% DESCRIPTION:
...
% - v6 created by DP, date uncertain
% - v7 created by DP on 6 Mar 2020. Takes trailing NaNs and does not include them in the
% calcualtion, but stables them back on at the end
%
...
% false:
use_native_func = 0; % set to 0 to handle edge cases more intelligently (and more slowly)
% notifications:
messages_on = 0;
warnings_on = 1;
...
%% ============================================================================================== %%
%% DERIVED QUANTITIES
% first, calculate number of trailing NaNs
n_inputs_init = length(input_spec);
n_end_nans = 0;
still_nans = 1; 
while still_nans
    ind_check = n_inputs_init - n_end_nans;
    if isnan(input_spec(ind_check))
        n_end_nans = n_end_nans + 1; % increment
    else 
        still_nans = 0; % exit while loop & don't increment
    end
end
n_inputs = n_inputs_init - n_end_nans; 
input_spec = input_spec(1:n_inputs); % adjust length
W = (frame_len - 1) / 2;

%% ============================================================================================== %%
%% ALLOCATE
if size(input_spec, 2) == 1 % if column vector
    input_spec = input_spec.'; % make row vector
end
output_spec_nanless = zeros(size(input_spec));
...
if freq_domain_fit == 1 % perform many, many polynomial fits
    ...
    for i = 1:n_inputs
        ...
        output_spec_nanless(i) = polyval(p, i); % i plays the role of the x-value we evaluate at
...
elseif freq_domain_fit == 2 % do the naive thing and apply the standard functionality without NaN's
    ...
    output_spec_nanless = input_spec(~isnan(input_spec)); % remove the NaN's
    % apply the filter:
    if use_native_func
        if warnings_on
        ...
        output_spec_nanless = sgolayfilt(output_spec_nanless, order, frame_len);
    else
        output_spec_nanless = FUNC_sgolayfilt_smartedge_v1(output_spec_nanless, order, frame_len);
    end
    % reinsert the NaN's:
    for j = 1:n_nans
        i = nan_locs(j); % index where the NaN was
        output_spec_nanless = [output_spec_nanless(1:i - 1), NaN, output_spec_nanless(i:end)];
    end
    
else % no frequency domain fit: apply filter in the time domain 
    output_spec_nanless = input_spec; % initialzie as the input spectrum
...
% make a version where the output spectrum NaNs where the input spectrum did:
output_spec = [output_spec_nanless, NaN(1, n_end_nans)]; % append back on the end NaNs
% new with v7, even the "nanless" one has trailing NaNs (but not intermediate ones), now: 
output_spec_nanless = [output_spec_nanless, NaN(1, n_end_nans)]; 
